...
#Run Case Composite
l=0
while l<1:#len(load_Bus): # changing the P load at bus 1 #test just going to 2
	k = l+1
	...
	psspy.fact()
	psspy.tysl(0)
	psspy.dyre_new([1,1,1,1],r"""C:\Users\psse\Desktop\Phylicia\Transmission V Distribution Study\Poland\CLM_test_2000_no_protection.dyr""","","","")
	...
	psspy.dist_bus_fault(1,1, 220,[0.0,-0.2E+10])
	#psspy.dist_bus_fault(k,1, bus_volt[l],[0.0,-0.2E+10])
	#psspy.load_chng_5(bus_num,r"""1""",[_i,_i,_i,_i,_i,_i,_i],[_f,_f, _f,_f,0.0,_f,_f,_f]) #load reductions results in very little change #no affect with the 10%, try zero
	...
	#Gather the data
	data = dyntools.CHNF(r"C:\Users\psse\Desktop\Phylicia\Transmission V Distribution Study\Poland\Channels\CLM\CLM_poland_BF_%d.outx" % l) #getting data from channel.out file
	...
	filename = "BusFault_CLM_poland_%d.csv" % l #there will be an excel file for every load bus change
	with open(filename,'w') as g:
		g.write("%s" % all)
	#np.savetxt(filename,all,delimiter=",") 
	
	#Add together to one giant matrix #not enough memory
	#if l==0:
	#	total=all
	#else:
